...
export class MetaAdsClient {
  ...
  async getAdCreative(adId: string): Promise<object> {
    const api = this.ensureApi();
  ...
  async getLeads(formId: string, params: LeadParams = {}): Promise<Lead[]> {
    const api = this.ensureApi();
  ...
  async createCustomAudience(
    ...
    if (data.dataSource) params.data_source = JSON.stringify(data.dataSource);
    if (data.audienceSize) params.audience_size = data.audienceSize;
  ...
  async getBudgetOptimization(
    campaignId: string,
    params: { targetCpa?: number; dailyBudget?: number }
  ): Promise<object> {
    const api = this.ensureApi();
